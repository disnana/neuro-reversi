...
function App() {
  ...
  const [concurrency, setConcurrency] = useState(Math.max(1, Math.floor(hardwareCores / 4)));
  ...
  const handleLimitBlur = async () => {
    ...
    await setMemoryLimit(val);
    setMemoryLimitState(val);
    await refreshStats();
  };

  const startTraining = async (mode: GameMode) => {
    // 1. Reset Session
    gameIdRef.current += 1;
  ...
  const handleWorkerMessage = async (message: WorkerOutputMessage, mode: GameMode, workerIndex: number, sessionId: number) => {
    ...
    if (message.type === 'GAME_OVER') {
      ...
      const currentBrain = await loadBrain();
      const updatedBrain = trainBrain(currentBrain, history, winner, mode);

      try {
        await saveBrain(updatedBrain);
      } catch (e) {
        console.error("Critical DB Error during save:", e);
      }
      ...
      if (winner) {
        // Stats are updated atomically inside trainBrain now to correctly handle Self Play vs Random
  ...
  const executeMove = async (row: number, col: number) => {
    ...
    if (nextMoves.length > 0) {
      ...
      if (originalColorMoves.length > 0) {
        ...
        setBoard(newBoard);
        // Game Over state will be picked up by the useEffect loop
        return;
      }
    }
  };
  ...
  useEffect(() => {
    ...
    const runTurn = async () => {
      ...
      if (isBotTurn) {
        ...
        setTimeout(async () => {
          ...
          const bestMove = await getBestMove(board, currentMoves);
          await executeMove(bestMove.row, bestMove.col);
          setIsProcessing(false);
        }, delay);
    ...
    // Check game state for everyone (Game Over detection), but only bots will auto-execute
  ...
  const confirmReset = async () => {
    ...
    setStats([]);
    setProcessedGames(0);
    setPatternCount(0); // Explicitly reset UI patterns
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center p-3 bg-slate-800 rounded-lg border border-slate-700">
                ...
                <span className="font-mono text-cyan-400 font-bold">
                  {stats.length > 0 ? stats[stats.length - 1].experience : 0}
                </span>
              </div>
              ...
              <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700 space-y-3">
                <div>
                  ...
                  <div className="text-[10px] text-slate-500 mt-1 text-center">
                    {concurrency >= hardwareCores ? "Max Capacity Reached" : "Safe Zone"}
                  </div>
                </div>
        ...
        <div className="lg:col-span-8 min-h-[500px] md:min-h-[600px] order-1 lg:order-2">
          {isParallelView ? (
            ...
            <div className="h-full flex flex-col items-center justify-center bg-slate-900/30 border border-slate-800 rounded-xl border-dashed p-6">
              ...
              {/* Mobile Quick Start Actions (Hidden on Large Screens) */}
              <div className="grid grid-cols-1 gap-3 w-full max-w-xs lg:hidden">
                <button
                  onClick={() => startTraining(GameMode.PVP)}
                  className="w-full py-4 bg-cyan-600 active:bg-cyan-500 rounded-xl font-bold text-white shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2"
                >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => startTraining(GameMode.TRAINING_RANDOM)}
                    className="py-3 bg-slate-800 active:bg-slate-700 rounded-lg text-xs font-bold text-slate-300 border border-slate-700"
                  >
                    VS RANDOM
                  </button>
                  <button
              ...
              <div className="hidden lg:block text-slate-600 text-sm">
                Select a mode from the control panel to begin.
              </div>
